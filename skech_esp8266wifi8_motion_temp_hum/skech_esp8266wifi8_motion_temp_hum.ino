...
void setup() {
  ...
  WiFi.mode(WIFI_STA);
  wifiMulti.addAP("YOUR_NETWORK_ID", "YOUR_WIFI_PASSWORD");
...
void loop() {
  if (wifiMulti.run() != WL_CONNECTED) {
    ...
    HTTPClient http;
    String url = "http://your_site.pythonanywhere.com/add_data?temp=" + temperature + "&humid=" + humidity + "&ip=" + broadCast + "&motion=" + motion;
